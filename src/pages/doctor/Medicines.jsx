  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        <h1 className="text-3xl font-bold bg-gradient-to-r from-blue-600 to-cyan-600 bg-clip-text text-transparent">Medicine Management</h1>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center" onClick={()=>setShowAddRoute(false)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md ring-1 ring-slate-200 overflow-hidden" onClick={e=>e.stopPropagation()}>
            ...
            <div className="px-6 py-4">
              ...
              <input value={newRoute} onChange={e=>setNewRoute(e.target.value)} placeholder="e.g., Nebulization, Topical" className="h-11 px-3 rounded-lg border-2 border-slate-200 focus:border-blue-400 focus:ring-4 focus:ring-blue-100 w-full transition-all duration-200" />
            </div>
            <div className="px-6 py-4 bg-slate-50 flex justify-end gap-2">
              ...
              <button onClick={handleAddRouteSave} className="h-9 px-3 rounded-lg bg-blue-600 hover:bg-blue-700 text-white cursor-pointer">Save</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="rounded-2xl bg-gradient-to-br from-white to-slate-50 shadow-xl ring-1 ring-slate-200/50 p-6 border border-slate-100">
        <form onSubmit={save} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="md:col-span-2">
              <label className="block text-sm font-semibold text-slate-700 mb-2 flex items-center gap-2">
                <svg className="w-4 h-4 text-blue-500" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M3 3a1 1 0 000 2v8a2 2 0 002 2h2.586l-1.293 1.293a1 1 0 101.414 1.414L10 15.414l2.293 2.293a1 1 0 001.414-1.414L12.414 15H15a2 2 0 002-2V5a1 1 0 100-2H3zm11.707 4.707a1 1 0 00-1.414-1.414L10 9.586 8.707 8.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd"/></svg>
                Medical Condition
              </label>
              <input className="h-12 px-4 rounded-xl border-2 border-slate-200 focus:border-blue-400 focus:ring-4 focus:ring-blue-100 w-full transition-all duration-200 bg-white shadow-sm" placeholder="e.g., Gastroenteritis, Dehydration" value={form.condition} onChange={e=>setForm(s=>({...s,condition:e.target.value}))} required />
            </div>
            <div className="md:col-span-1">
              <label className="block text-sm font-semibold text-slate-700 mb-2 flex items-center gap-2">
                <svg className="w-4 h-4 text-blue-500" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M8 4a4 4 0 100 8 4 4 0 000-8zM2 8a6 6 0 1110.89 3.476l4.817 4.817a1 1 0 01-1.414 1.414l-4.816-4.816A6 6 0 012 8z" clipRule="evenodd"/></svg>
                Search Regimens
              </label>
              <input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search conditions, medicines..." className="h-12 px-4 rounded-xl border-2 border-slate-200 focus:border-blue-400 focus:ring-4 focus:ring-blue-100 w-full transition-all duration-200 bg-white shadow-sm" />
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            {form.rows.map((r, idx)=> (
              <div key={idx} className="group rounded-xl border-2 border-slate-200 hover:border-blue-300 p-4 bg-gradient-to-br from-white to-blue-50/30 shadow-sm hover:shadow-md transition-all duration-200">
                {/* Line 1 */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div>
                    ...
                    }} className="h-11 px-3 rounded-lg border-2 border-slate-200 focus:border-blue-400 focus:ring-4 focus:ring-blue-100 bg-white w-full transition-all duration-200">
                  ...
                  <div>
                    ...
                    <input value={r.name} onChange={e=>updateRow(idx,'name',e.target.value)} placeholder="Enter medicine name" className="h-11 px-3 rounded-lg border-2 border-slate-200 focus:border-blue-400 focus:ring-4 focus:ring-blue-100 w-full transition-all duration-200" />
                  </div>
                </div>
                {/* Line 2 */}
                <div className="grid grid-cols-1 md:grid-cols-5 gap-3 mt-3">
                  <div>
                    ...
                    <input value={r.doseRate||''} onChange={e=>updateRow(idx,'doseRate',e.target.value)} placeholder="e.g., 10 mg/kg" className="h-11 px-3 rounded-lg border-2 border-slate-200 focus:border-blue-400 focus:ring-4 focus:ring-blue-100 w-full transition-all duration-200" />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-2">Composition per ml</label>
                    <input value={r.perMl||''} onChange={e=>updateRow(idx,'perMl',e.target.value)} placeholder="e.g., 100 mg/ml" className="h-11 px-3 rounded-lg border-2 border-slate-200 focus:border-blue-400 focus:ring-4 focus:ring-blue-100 w-full transition-all duration-200" />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-2">Unit</label>
                    <input value={r.unit} onChange={e=>updateRow(idx,'unit',e.target.value)} placeholder="ml, mg, tabs" className="h-11 px-3 rounded-lg border-2 border-slate-200 focus:border-blue-400 focus:ring-4 focus:ring-blue-100 w-full transition-all duration-200" />
                  </div>
                  <div className="md:col-span-2">
                    <label className="block text-sm font-semibold text-slate-700 mb-2">Instructions</label>
                    <input value={r.instructions} onChange={e=>updateRow(idx,'instructions',e.target.value)} placeholder="e.g., once daily for 5 days" className="h-11 px-3 rounded-lg border-2 border-slate-200 focus:border-blue-400 focus:ring-4 focus:ring-blue-100 w-full transition-all duration-200" />
                  </div>
                </div>
          ...
          <button type="button" onClick={addRow} className="h-12 px-6 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 text-white font-semibold cursor-pointer transition-all duration-200 shadow-lg hover:shadow-xl flex items-center gap-2">
          ...
          <div className="flex gap-3">
            <button className="px-6 h-12 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 text-white font-semibold cursor-pointer transition-all duration-200 shadow-lg hover:shadow-xl flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {filtered.map(m=> (
          <div key={m.id} className="group rounded-2xl bg-gradient-to-br from-white to-slate-50 border-2 border-slate-200 hover:border-blue-300 shadow-lg hover:shadow-xl p-6 transition-all duration-300 hover:scale-105">
            <div className="flex items-start justify-between mb-3">
              ...
              <div className="flex gap-2">
                <button onClick={()=>edit(m)} className="px-3 h-9 rounded-lg bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 text-white cursor-pointer text-sm font-medium transition-all duration-200 shadow-md hover:shadow-lg flex items-center gap-1">
            ...
            <div className="space-y-3">
              {(m.rows||[]).map((r,i)=> (
                <div key={i} className="bg-gradient-to-r from-slate-50 to-blue-50/50 rounded-lg p-3 border border-slate-200">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="px-2 py-1 bg-blue-100 text-blue-700 rounded-md text-xs font-semibold">{r.route}</span>
                    <span className="font-semibold text-slate-800">{r.name}</span>
                  </div>
                  <div className="text-sm text-slate-600 flex flex-wrap gap-x-3">
                    {r.doseRate && <span className="flex items-center gap-1"><svg className="w-3 h-3 text-blue-500" fill="currentColor" viewBox="0 0 20 20"><circle cx="10" cy="10" r="3"/></svg>Dose Rate: {r.doseRate}</span>}
